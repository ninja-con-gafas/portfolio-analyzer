...
PORTFOLIO_ANALYZER_APPLICATION_CONFIGURATION_DIRECTORY="config"
PORTFOLIO_ANALYZER_APPLICATION_PROCESS_DIRECTORY="process"
...
USER airflow
RUN pip install --no-cache-dir psycopg2-binary pyspark
EOF
...
services:
  airflow-scheduler:
    ...
    environment:
      ...
      AIRFLOW_CONN_PORTFOLIOANALYZER_DB: $AIRFLOW_CONN_PORTFOLIOANALYZER_DB
    networks:
      - $DOCKER_NETWORK_NAME
    volumes:
      - ./$AIRFLOW_DAG_DIRECTORY:/opt/airflow/dags
      - ./$PORTFOLIO_ANALYZER_APPLICATION_CONFIGURATION_DIRECTORY:/opt/airflow/config
      - ./$PORTFOLIO_ANALYZER_APPLICATION_PROCESS_DIRECTORY:/opt/airflow/process
